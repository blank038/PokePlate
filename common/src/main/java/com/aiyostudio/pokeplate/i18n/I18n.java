package com.aiyostudio.pokeplate.i18n;
...
public class I18n {
    ...
    public void init(String language) {
        ...
        File folder = new File(PokePlate.getInstance().getDataFolder(), "language");
        if (!folder.exists()) {
            folder.mkdir();
        ...
        for (String key : data.getKeys(true)) {
            if (data.isString(key)) {
                this.stringOptions.put(key, TextUtil.colorify(data.getString(key, "")));
            } else if (data.isList(key)) {
                this.arrayOptions.put(key, data.getStringList(key).stream()
                        .map(TextUtil::colorify)
